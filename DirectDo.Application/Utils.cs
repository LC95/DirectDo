...
namespace DirectDo.Application
{
    public static class Utils
    {
        ...
        public static IControlCommand BuildCommand(DeleteOptions deleteOptions)
        {
            return new DeleteCommand(deleteOptions.ReqId);
        }

        public static IControlCommand BuildCommand(LookOptions lookOptions)
        {
        ...
        public static Command CreateAddCommand(Func<AddOptions, Task> func)
        {
        ...
        public static Command CreateDeleteCommand(Func<DeleteOptions, Task> func)
        {
            var deleteCommand = new Command("delete", "删除一个闹钟, 未完成");
            deleteCommand.AddArgument(new Argument("reqid"));
            deleteCommand.Handler = CommandHandler.Create(func);
            return deleteCommand;
        }

        public static Command CreateSearchCommand(Func<LookOptions, Task> func)
        {
            var searchCommand = new Command("look", "搜索全部闹钟")
            {
                Handler = CommandHandler.Create(func)
            };
            searchCommand.AddArgument(new Argument("text"));
